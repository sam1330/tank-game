...
public class GameManager: MonoBehaviour {
  ...
  public Text timerText;  // Reference to the Text component displaying the time
  ...
  private void Update() {
    if (currentTime > 0)
    {
        currentTime -= Time.deltaTime;  // Reduce time by the elapsed time since the last frame
        UpdateTimerText();
    }
    else
    {
        // Timer has reached 0, you can implement game over logic or other actions here
  ...
  private string EndMessage() {
    // Por defecto no hya ganadores, así que es empate.
